...
class SMNNetwork(TensorflowBaseMatchingModel):
    ...
    def _init_graph(self):
        ...
        all_utterance_embeddings = tf.nn.embedding_lookup(params=word_embeddings, ids=self.utterance_ph)
        response_embeddings = tf.nn.embedding_lookup(params=word_embeddings, ids=self.response_ph)
        sentence_GRU = tf.compat.v1.nn.rnn_cell.GRUCell(self.word_embedding_size, kernel_initializer=tf.compat.v1.orthogonal_initializer())
        ...
        A_matrix = tf.compat.v1.get_variable('A_matrix_v', shape=(self.word_embedding_size, self.word_embedding_size),
                                   initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"), dtype=tf.float32)
        final_GRU = tf.compat.v1.nn.rnn_cell.GRUCell(self.word_embedding_size, kernel_initializer=tf.compat.v1.orthogonal_initializer())
        reuse = None

        response_GRU_embeddings, _ = tf.compat.v1.nn.dynamic_rnn(sentence_GRU,
                                                       response_embeddings,
                                                       sequence_length=self.response_len_ph,
                                                       dtype=tf.float32,
                                                       scope='sentence_GRU')
        response_embeddings = tf.transpose(a=response_embeddings, perm=[0, 2, 1])
        response_GRU_embeddings = tf.transpose(a=response_GRU_embeddings, perm=[0, 2, 1])
        matching_vectors = []
        for utterance_embeddings, utterance_len in zip(all_utterance_embeddings, all_utterance_len):
            ...
            utterance_GRU_embeddings, _ = tf.compat.v1.nn.dynamic_rnn(sentence_GRU,
            ...
            conv_layer = tf.compat.v1.layers.conv2d(matrix, filters=8, kernel_size=(3, 3), padding='VALID',
                                          kernel_initializer=tf.contrib.keras.initializers.he_normal(),
                                          activation=tf.nn.relu, reuse=reuse, name='conv')  # TODO: check other params
            pooling_layer = tf.compat.v1.layers.max_pooling2d(conv_layer, (3, 3), strides=(3, 3),
                                                    padding='VALID', name='max_pooling')  # TODO: check other params
            matching_vector = tf.compat.v1.layers.dense(tf.contrib.layers.flatten(pooling_layer), 50,
        ...
        _, last_hidden = tf.compat.v1.nn.dynamic_rnn(final_GRU,
                                           tf.stack(matching_vectors, axis=0, name='matching_stack'),
                                           # resulting shape: (10, ?, 50)
                                           dtype=tf.float32,
                                           time_major=True,
                                           scope='final_GRU')  # TODO: check time_major
        logits = tf.compat.v1.layers.dense(last_hidden, 2, kernel_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"),
                                 name='final_v')
        self.y_pred = tf.nn.softmax(logits)
        self.logits = logits
        self.loss = tf.reduce_mean(input_tensor=tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.y_true, logits=logits))
