...
def FFN(x, out_dimension_0=None, out_dimension_1=None):
    ...
    with tf.compat.v1.variable_scope('FFN_1'):
        y = op.dense(x, out_dimension_0, initializer=tf.compat.v1.keras.initializers.he_normal(seed=42))
        y = tf.nn.relu(y)
...
def CNN_3d(x, out_channels_0, out_channels_1, add_relu=True):
    ...
    weights_0 = tf.compat.v1.get_variable(
        ...
        dtype=tf.float32,
        initializer=tf.compat.v1.random_uniform_initializer(-0.001, 0.001))
