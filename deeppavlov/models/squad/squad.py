...
class SquadModel(LRScheduledTFModel):
    ...
    def _init_graph(self):
        ...
        self.word_emb = tf.compat.v1.get_variable("word_emb", initializer=tf.constant(self.init_word_emb, dtype=tf.float32),
                                        trainable=False)
        self.char_emb = tf.compat.v1.get_variable("char_emb", initializer=tf.constant(self.init_char_emb, dtype=tf.float32),
                                        trainable=self.train_char_emb)

        self.c_mask = tf.cast(self.c_ph, tf.bool)
        self.q_mask = tf.cast(self.q_ph, tf.bool)
        self.c_len = tf.reduce_sum(input_tensor=tf.cast(self.c_mask, tf.int32), axis=1)
        self.q_len = tf.reduce_sum(input_tensor=tf.cast(self.q_mask, tf.int32), axis=1)

        bs = tf.shape(input=self.c_ph)[0]
        self.c_maxlen = tf.reduce_max(input_tensor=self.c_len)
        self.q_maxlen = tf.reduce_max(input_tensor=self.q_len)
        ...
        self.cc_len = tf.reshape(tf.reduce_sum(input_tensor=tf.cast(tf.cast(self.cc, tf.bool), tf.int32), axis=2), [-1])
        self.qc_len = tf.reshape(tf.reduce_sum(input_tensor=tf.cast(tf.cast(self.qc, tf.bool), tf.int32), axis=2), [-1])
        ...
        with tf.compat.v1.variable_scope("emb"):
            with tf.compat.v1.variable_scope("char"):
                cc_emb = tf.reshape(tf.nn.embedding_lookup(params=self.char_emb, ids=self.cc),
                                    [bs * self.c_maxlen, self.char_limit, self.char_emb_dim])
                qc_emb = tf.reshape(tf.nn.embedding_lookup(params=self.char_emb, ids=self.qc),
            ...
            with tf.compat.v1.name_scope("word"):
                c_emb = tf.nn.embedding_lookup(params=self.word_emb, ids=self.c)
                q_emb = tf.nn.embedding_lookup(params=self.word_emb, ids=self.q)
        ...
        with tf.compat.v1.variable_scope("match"):
        ...
        with tf.compat.v1.variable_scope("pointer"):
            ...
            if self.noans_token:
                noans_token = tf.Variable(tf.random.uniform((match.get_shape().as_list()[-1],), -0.1, 0.1), tf.float32)
                noans_token = tf.nn.dropout(noans_token, rate=1 - (self.keep_prob_ph))
        ...
        with tf.compat.v1.variable_scope("predict"):
            ...
            self.yp1 = tf.argmax(input=tf.reduce_max(input_tensor=outer, axis=2), axis=1)
            self.yp2 = tf.argmax(input=tf.reduce_max(input_tensor=outer, axis=1), axis=1)
            ...
            loss_1 = tf.nn.softmax_cross_entropy_with_logits(logits=logits1, labels=tf.stop_gradient(self.y1))
            loss_2 = tf.nn.softmax_cross_entropy_with_logits(logits=logits2, labels=tf.stop_gradient(self.y2))
            self.loss = tf.reduce_mean(input_tensor=loss_1 + loss_2)

    def _init_placeholders(self):
        self.c_ph = tf.compat.v1.placeholder(shape=(None, None), dtype=tf.int32, name='c_ph')
        self.cc_ph = tf.compat.v1.placeholder(shape=(None, None, self.char_limit), dtype=tf.int32, name='cc_ph')
        self.q_ph = tf.compat.v1.placeholder(shape=(None, None), dtype=tf.int32, name='q_ph')
        self.qc_ph = tf.compat.v1.placeholder(shape=(None, None, self.char_limit), dtype=tf.int32, name='qc_ph')
        self.y1_ph = tf.compat.v1.placeholder(shape=(None,), dtype=tf.int32, name='y1_ph')
        self.y2_ph = tf.compat.v1.placeholder(shape=(None,), dtype=tf.int32, name='y2_ph')

        self.lear_rate_ph = tf.compat.v1.placeholder_with_default(0.0, shape=[], name='learning_rate')
