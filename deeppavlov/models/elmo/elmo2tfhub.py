...
def make_module_spec(options, weight_file):
    ...
    def module_fn():
        ...
        def token2ids(token):
            with tf.compat.v1.name_scope("token2ids_preprocessor"):
                char_ids = tf.io.decode_raw(token, tf.uint8, name='decode_raw2get_char_ids')
                ...
                ids_num = tf.shape(input=char_ids)[0]
        ...
        def sentence_tagging_and_padding(sen_dim):
            with tf.compat.v1.name_scope("sentence_tagging_and_padding_preprocessor"):
                sen = sen_dim[0]
                dim = sen_dim[1]
                extra_dim = tf.shape(input=sen)[0] - dim
                ...
                bos_sen_eos_pads = tf.pad(tensor=bos_sen_eos_plus_one, paddings=[[0, extra_dim], [0, 0]],
                                          mode="CONSTANT", name='pad2bos_sen_eos_pads')
                return bos_sen_eos_pads

        # Input placeholders to the biLM.
        tokens = tf.compat.v1.placeholder(shape=(None, None), dtype=tf.string, name='ph2tokens')
        sequence_len = tf.compat.v1.placeholder(shape=(None,), dtype=tf.int32, name='ph2sequence_len')

        tok_shape = tf.shape(input=tokens)
        ...
        # Input placeholders to the biLM.
        def_strings = tf.compat.v1.placeholder(shape=(None), dtype=tf.string)
        def_tokens_sparse = tf.compat.v1.string_split(def_strings)
        def_tokens_dense = tf.compat.v1.sparse_to_dense(sparse_indices=def_tokens_sparse.indices,
        ...
        def_sequence_len = tf.reduce_sum(input_tensor=def_int_mask, axis=-1)

        def_tok_shape = tf.shape(input=def_tokens_dense)
