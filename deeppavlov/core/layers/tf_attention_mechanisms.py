...
def general_attention(key, context, hidden_size, projected_align=False):
    ...
    batch_size = tf.shape(input=context)[0]
    ...
    projected_key = \
        tf.compat.v1.layers.dense(key, hidden_size, kernel_initializer=xav())
    r_projected_key = tf.reshape(projected_key, shape=[-1, hidden_size, 1])

    lstm_fw_cell = tf.compat.v1.nn.rnn_cell.LSTMCell(hidden_size // 2)
    lstm_bw_cell = tf.compat.v1.nn.rnn_cell.LSTMCell(hidden_size // 2)
    (output_fw, output_bw), states = \
        tf.compat.v1.nn.bidirectional_dynamic_rnn(cell_fw=lstm_fw_cell,
    ...
    attn = tf.nn.softmax(tf.matmul(bilstm_output, r_projected_key), axis=1)

    if projected_align:
        log.info("Using projected attention alignment")
        t_context = tf.transpose(a=bilstm_output, perm=[0, 2, 1])
        output = tf.reshape(tf.matmul(t_context, attn),
                            shape=[batch_size, -1, hidden_size])
    else:
        log.info("Using without projected attention alignment")
        t_context = tf.transpose(a=r_context, perm=[0, 2, 1])
        output = tf.reshape(tf.matmul(t_context, attn),
...
def light_general_attention(key, context, hidden_size, projected_align=False):
    ...
    # projected_key: [None, None, hidden_size]
    projected_key = tf.compat.v1.layers.dense(key, hidden_size, kernel_initializer=xav())
    r_projected_key = tf.reshape(projected_key, shape=[-1, hidden_size, 1])

    # projected context: [None, None, hidden_size]
    projected_context = \
        tf.compat.v1.layers.dense(r_context, hidden_size, kernel_initializer=xav())

    attn = tf.nn.softmax(tf.matmul(projected_context, r_projected_key), axis=1)

    if projected_align:
        log.info("Using projected attention alignment")
        t_context = tf.transpose(a=projected_context, perm=[0, 2, 1])
        output = tf.reshape(tf.matmul(t_context, attn),
                            shape=[batch_size, -1, hidden_size])
    else:
        log.info("Using without projected attention alignment")
...
def cs_general_attention(key, context, hidden_size, depth, projected_align=False):
    ...
    if hidden_size % 2 != 0:
        raise ValueError("hidden size must be dividable by two")
    key_size = tf.shape(input=key)[-1]
    batch_size = tf.shape(input=context)[0]
    max_num_tokens, token_size = context.get_shape().as_list()[-2:]
    r_context = tf.reshape(context, shape=[-1, max_num_tokens, token_size])
    # projected_context: [None, max_num_tokens, token_size]
    projected_context = tf.compat.v1.layers.dense(r_context, token_size,
...
def bahdanau_attention(key, context, hidden_size, projected_align=False):
    ...
    projected_state = \
        tf.compat.v1.layers.dense(concat_h_state, hidden_size, use_bias=False,
                        kernel_initializer=xav())
    score = \
        tf.compat.v1.layers.dense(tf.tanh(projected_state), units=1, use_bias=False,
                        kernel_initializer=xav())
